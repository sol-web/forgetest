...
var userAPrivateKey = pki.privateKeyFromPem(userAPrivateKeyPem) //pem에서 개인키를 만듬
...
md.update(ID + time, 'utf8') //서명은 바이너리 객체이나 보낼 때는 핵사로 바꿔서 보냄
...
console.log('Signature:' + signatureHex) //모든 값을 받았다고 가정 // 2.1 사용자 A로부터 <ID, time, signature>를 수신
...
var userACert = pki.certificateFromPem(userACertPem) //certificateFromPem에서 userACertPem읽어옴, 검증-인증서 읽어와야 함
...
console.log('1. 사용자 A의 인증 검증: ' + verified1) //서버가 검증

// 2.5 전자서명 검증
var userAPublicKey = userACert.publicKey //userA의 공개키
var signature1 = forge.util.hexToBytes(signatureHex)
var md = forge.md.sha1.create()
md.update(ID + time, 'utf8')
var verified2 = userAPublicKey.verify(md.digest().bytes(), signature) //signatureHex를 받아 sivnature로 바꿈
console.log('2. 서명의 유효성 검증: ' + verified2)

var timeS = new Date().getTime() //서버의 현재 시간
console.log('서버 현재시간: ' + timeS)
var timeDiff = timeS - time
console.log('3. 현재시간의 유효성 검증: ' + timeDiff / 1000 + '초')
var verified3
if (timeDiff < 1000) verified3 = true // 몇초 이내면 인증
