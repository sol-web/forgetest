...
const fs = require('fs')
const pki = forge.pki //const 같은 이름을 다른곳에서 쓰지 않기에 사용
...
// 1.3 인증기관의 인증서 읽어옴
const caCertPem = fs.readFileSync('rootCert.pem', 'utf8') //caCertPem으로 읽어옴
...
console.log('1.7 keyString', keyString) // 1.7 세션키를 수신자 B의 공개키로 암호화
...
// 송신자가 수신자에게 전송: <encryptedHex, encryptedHexSessionKeyHex>
//  encryptedHex: 세션키로 암호화 된 메시지
// encryptedHexSessionKeyHex : 수신자의 공개키로 암호화한 세션키
...
let keyObject1 = JSON.parse(keyString)
let key1 = forge.util.hexToBytes(keyObject1.key) //hex를 다시 bytes로
...
let verified = userAPublicKey.verify(md.digest().bytes(), signature1) //검증
